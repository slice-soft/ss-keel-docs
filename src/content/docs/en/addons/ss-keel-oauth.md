...
The `cmd/setup_oauth.go` generated by `keel add oauth` reads credentials for all three providers, builds callback URLs from `OAUTH_REDIRECT_BASE_URL`, and only activates providers that have complete credentials. `OAUTH_ENABLED_PROVIDERS` can further restrict which routes are exposed.
