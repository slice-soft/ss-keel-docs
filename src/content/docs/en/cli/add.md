...
| `main_code` | Inserts code before `app.Listen()` in `cmd/main.go`; `guard` avoids duplicates; `anchor` field accepted (currently only `"before_listen"`) |
| `create_provider_file` | Creates a new Go file (e.g. `cmd/setup_database.go`) with a self-contained setup function, keeping `cmd/main.go` clean; `guard` checks for the function signature before creating |

Unknown step types fail the install.

### `create_provider_file` pattern

Instead of bloating `cmd/main.go` with initialization code, addons use this step to generate a dedicated file. For example, `keel add gorm` creates `cmd/setup_database.go`:

```go
// cmd/setup_database.go — generated by keel add gorm
package main
